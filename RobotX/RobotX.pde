...
void mouseReleased(){
  if((logo2.mouseOverButton() && pantalla == Pantalles.INICI  && logo2.enabled) ||
    ...
    int ide = getMaxIdExperiencia();
    println("ID MAX: "+ide);
    for(String s: materialsCreate){
      println(s);
      if(createMaterials.isChecked(s)){
        int idc = getIdFromComponente(s);
        println(idc);
        insertComponenteExperiencia(ide, idc, 1);
      }
    }
    for(String f: namesImages){
      if(f != null){
        insertFotoExperiencia(ide, f);
      }
    }
